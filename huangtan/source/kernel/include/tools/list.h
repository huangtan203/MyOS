...
//求node节点所在结构体首地址
#define offset_to_parent(parent_type,node_name) \
    ((uint32_t)node_name - offset_in_parent(parent_type,node_name))
//进行转换
#define list_node_parent(node,parent_type,node_name) \
    ((parent_type*)(node?offset_to_parent(parent_type,node_name):0))
...
static inline list_t*list_first(list_t *list){
    return list->first;
}
